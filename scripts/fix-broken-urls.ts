...
// Verified working state URLs
const VERIFIED_STATE_URLS = {
  'Alabama': 'https://www.adeca.alabama.gov/',
...
async function fixBrokenUrls() {
  console.log('🔧 Fixing broken URLs with verified working ones...')
  ...
  for (const program of statePrograms) {
    const verifiedUrl = VERIFIED_STATE_URLS[program.state.name as keyof typeof VERIFIED_STATE_URLS]
    
    if (verifiedUrl && verifiedUrl !== program.url) {
      await prisma.program.update({
        where: { id: program.id },
        data: { url: verifiedUrl }
      })
      
      updatedCount++
      console.log(`✅ ${program.title}: ${program.url} → ${verifiedUrl}`)
    }
  }
  
  console.log(`🎉 Updated ${updatedCount} state grants with verified working URLs!`)
  return updatedCount
}
...
fixBrokenUrls()
  .then(async (count) => {
    console.log(`✅ URL fix completed. Updated ${count} grants.`)
